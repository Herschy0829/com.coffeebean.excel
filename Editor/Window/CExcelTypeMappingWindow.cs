...
namespace CoffeeBean
{
    ...
    public sealed class CExcelTypeMappingWindow : EditorWindow
    {
        ...
        private void DrawRules()
        {
            ...
            Label("数组分隔符", "默认 " + CExcelCellJson.DefaultArraySeparators + "（含 `_`：`13_100` = [13,100]，真实项目最常见）—— " +
                                "可在「Excel 配置表工具」的生成选项里改；**例外**：向量/颜色/矩形/四元数只能用 ; 或 |（元素内部是逗号），" +
                                "字典数组 _kva 用 |，枚举数组不按 _ 拆（`_` 是 \"名字_值\" 语法）；空单元格 → 空数组");
            Label("空值", "数值空 → 0；字符串空 → 空串；字符空 → \\0；时间/标识空 → 零值；结构空 → 全 0；枚举空 → 0；数组空 → 空数组");
            Label("表头", "自动在前 3 行里找\"带后缀列名最多\"的那行当表头；它上方最近一行作为字段注释");
            Label("说明行", "**主键列空着的行会被跳过**（说明行 / 图例行 / 边上的草稿块），并出一条警告列出跳过的行号；" +
                            "整张表主键全空则报错（多半是表头检测选错了行）。可在生成选项里关掉");
